...
const InteractiveExercises = ({ listeningQuestions }: InteractiveExercisesProps) => {
  ...
  // Estado para o exercício de listening com múltipla escolha
  const [listeningAnswers, setListeningAnswers] = useState<number[]>(
    listeningQuestions ? Array(listeningQuestions.length).fill(-1) : []
  );
  ...
  // Função para atualizar as respostas do listening
  const handleListeningAnswer = (questionIndex: number, optionIndex: number) => {
    const newAnswers = [...listeningAnswers];
    newAnswers[questionIndex] = optionIndex;
    setListeningAnswers(newAnswers);
  };
  
  // Função para verificar as respostas do listening
  const checkListeningAnswers = () => {
    setShowListeningFeedback(true);
    const feedbackElement = document.getElementById('listening-mcq-feedback');
    if (feedbackElement) {
      window.scrollTo({
        top: feedbackElement.offsetTop - 100,
        behavior: 'smooth'
  ...
  // Função para calcular a pontuação do listening
  const calculateListeningScore = () => {
    if (!listeningQuestions) return 0;
    
    return listeningAnswers.filter((answer, index) => 
      answer === listeningQuestions[index].correctOptionIndex
    ).length;
  };
  
  // Função para resetar as respostas do listening
  const resetListeningAnswers = () => {
    setListeningAnswers(Array(listeningQuestions?.length || 0).fill(-1));
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Exercício 5: Listening (se disponível) */}
      {listeningQuestions && listeningQuestions.length > 0 && (
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
          <h3 className="text-2xl font-bold text-purple-700 mb-4">Exercício 5: Compreensão Auditiva</h3>
          ...
          <div className="space-y-6">
              ...
              <div key={qIndex} className="bg-purple-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    <button
                      ...
                      onClick={() => handleListeningAnswer(qIndex, oIndex)}
                    >
                      {option}
                    </button>
                  ))}
            ...
            {/* Feedback for Listening MCQ */}
            {showListeningFeedback && (
              <div id="listening-mcq-feedback" className="mt-6 p-6 bg-purple-50 border-l-4 border-purple-400 rounded-lg shadow-md">
                <h4 className="text-xl font-semibold mb-3 text-purple-800">Resultado:</h4>
                <p className="text-lg mb-4">
                  Você acertou <span className="font-bold text-purple-700">{calculateListeningScore()}</span> de {listeningQuestions.length} perguntas!
                </p>
                <div className="space-y-4">
                  {listeningQuestions.map((question, qIndex) => {
                    const isCorrect = listeningAnswers[qIndex] === question.correctOptionIndex;
                    return (
                      <div key={qIndex} className={`p-4 rounded-md ${isCorrect ? 'bg-green-100' : 'bg-red-100'}`}>
                        <p className="flex items-start">
                          <span className={`mr-2 mt-1 ${isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                          ...
                          <span className="flex-grow">
                            ...
                            <span className="block mt-1">
                              Sua resposta: {listeningAnswers[qIndex] >= 0 ? question.options[listeningAnswers[qIndex]] : 'Nenhuma resposta selecionada'}
                            </span>
                            <span className="block mt-1">
